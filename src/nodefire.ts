...
> {
  ...
  child<P extends string>(
    path: P, scope?: Scope
  ): NodeFire<ResultFor<Root, P>, WriteSpecialRules, ResultFor<WriteRoot, P>>;

  child<const Paths extends readonly string[]>(
    path: string, scope: Scope, typePaths: Paths
  ): NodeFire<
    ResultFor<Root, Paths[number]>,
    WriteSpecialRules,
    ResultFor<WriteRoot, Paths[number]>
  >;
  ...
  * when the path may contain interpolation syntax that must be disregarded, and you've already
  ...
  transaction<T extends WriteRoot | null | undefined>(
    updateFunction: (value: Root | null) => T,
    options?: {
      detectStuck?: number,
      prefetchValue?: boolean,
      timeout?: number
    }
  ): Promise<Root | null | undefined> {
    ...
    return Promise.all(
      ...
      const promise = new Promise<Root | null | undefined>((resolve, reject) => {
...
export class Snapshot<Root> {
  ...
  child<P extends string>(path: P, scope: Scope): Snapshot<ResultFor<Root, P>> {
...
function getNormalValue<T>(snap: admin.database.DataSnapshot): T {
...
type PushValue<T> =
  NonNullable<T> extends {[key: string]: infer Child} ? Child : T;
